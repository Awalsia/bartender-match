import BartenderCard from "@/components/cards/BartenderCard";
...
export default function EmployerBrowseScreen() {
  ...
  const loadBartenders = useCallback(async () => {
    setLoading(true);
    setMessage("");
    setCurrentIndex(0);
    ...
    if (profileError) {
      console.log("LOAD BARTENDERS ERROR:", profileError);
      setMessage(profileError.message);
      setLoading(false);
      return;
    }

    const allProfiles = (profileRows ?? []) as BartenderProfileRow[];
    ...
    const bartenderIds = availableProfiles.map((profile) => profile.id);

    const { data: photoRows, error: photoError } = await supabase
      .from("bartender_photos")
      .select("bartender_id, url, is_cover, created_at")
    ...
    const photos = (photoRows ?? []) as BartenderPhotoRow[];

    const formattedBartenders: Bartender[] = availableProfiles.map(
      (profile) => {
        const profilePhotos = photos
          .filter((photo) => photo.bartender_id === profile.id)
    ...
    setBartenders(formattedBartenders);
    setLoading(false);
  }, [cardPosition]);
  ...
  async function saveSwipe(direction: SwipeDirection) {
    const currentBartender = bartenders[currentIndex];

    if (!currentBartender || !swiperId || decisionLoading) {
      return;
    }
    ...
    const { error } = await supabase.from("swipes").upsert(
      {
        swiper_id: swiperId,
        swiped_user_id: currentBartender.userId,
        direction,
      },
      {
    ...
    if (direction === "right" && !existedBefore) {
      ...
      if (matchCreated) {
        const fullName =
          [currentBartender.firstName, currentBartender.lastName]
            .filter(Boolean)
            .join(" ") || "Bartender";

        setNewMatch({
          name: fullName,
          photoUrl: currentBartender.coverPhotoUrl,
        });
      }
    }

    setMessage(
      direction === "right" ? "Bartender liked." : "Bartender skipped.",
    );
  ...
  function openBartenderProfile(bartenderId: string) {
    const route = {
  ...
  const panResponder = useMemo(
    ...
    [bartenders, currentIndex, decisionLoading, swiperId, newMatch],
  );
  ...
  if (loading) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.loadingText}>Loading bartenders...</Text>
      </View>
    );
  }
  ...
  if (!currentBartender) {
    return (
      <>
        <View style={styles.centerContainer}>
          <Text style={styles.emptyTitle}>No more bartenders</Text>

          <Text style={styles.emptyText}>
            You have reviewed all currently available bartender profiles.
          </Text>

          <Pressable style={styles.primaryButton} onPress={loadBartenders}>
        ...
        <MatchModal
          ...
          matchedName={newMatch?.name ?? "Bartender"}
    ...
    <>
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Discover bartenders</Text>

          <Text style={styles.counter}>
            {currentIndex + 1}/{bartenders.length}
          </Text>
        </View>

        <Text style={styles.subtitle}>
          Find the right bartender for your business.
        </Text>
        ...
        >
          ...
          <BartenderCard
            bartender={currentBartender}
            onPress={() => openBartenderProfile(currentBartender.id)}
          />
        </Animated.View>
